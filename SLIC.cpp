	...
	const double&				M)
{
	...
	double dist;
	double distxy;
	for (int itr = 0; itr < 10; itr++) //\BD\F8\D0\D010\B4ε\FC\B4\FA\B5ķָ\D7\EE\D6շָ\EE\CEȶ\A8
	{
	...
	{
		//cout << "\B3\AC\CF\F1\CBظ\F6\CA\FD\A3\BA" << clustersize[i] << endl;
	}
}
	...
	for (int j = 0; j < height; j++)
	{
		for (int k = 0; k < width; k++)
		{
			if (0 > nlabels[oindex])
			{
				...
				// adjacent label found before, and decrement label count.  //\B2\A2\C7Ҽ\F5\C9ٳ\AC\CF\F1\CBص\C4\CA\FD\C1\BF\BC\C6\CA\FD
				//-------------------------------------------------------
				if (count <= SUPSZ >> 2)
				{
					for (int c = 0; c < count; c++)
